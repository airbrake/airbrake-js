...
describe "FallbackProcessor", ->
  describe "#process", ->
    ...
    it 'is named fallback', (finish) ->
      processor.process {}, (name, result) ->
        expect(name).to.equal('fallback')
        do finish

    it "determines error type", (finish) ->
      processor.process stack: "SyntaxError: unexpected }", (name, result) ->
